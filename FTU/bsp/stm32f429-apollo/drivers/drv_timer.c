...
static TIM_HandleTypeDef TIM4_Handler; // 输入捕获
...
void TIM3_CH2_Cap_Init(uint32_t arr,uint16_t psc)
{
    ...
    TIM3_CH2Config.ICPolarity=TIM_ICPOLARITY_RISING;    //上升沿捕获，捕获极性
    TIM3_CH2Config.ICSelection=TIM_ICSELECTION_DIRECTTI;
    TIM3_CH2Config.ICPrescaler=TIM_ICPSC_DIV1;          //配置输入分频，不分频，每个上升沿都会捕获
    TIM3_CH2Config.ICFilter=0;
    HAL_TIM_IC_ConfigChannel(&TIM3_Handler,&TIM3_CH2Config,TIM_CHANNEL_2);

    HAL_TIM_IC_Start_IT(&TIM3_Handler,TIM_CHANNEL_2);   // 开启TIM5的捕获通道1，并且开启捕获中断
    __HAL_TIM_ENABLE_IT(&TIM3_Handler,TIM_IT_UPDATE); // 使能更新中断,不使用更新中断，因为定时器预装值已经足够大了
    // 所谓更新中断就是普通的中断处理函数
}

void TIM4_CH1_Cap_Init(uint32_t arr,uint16_t psc)
{
    ...
    TIM4_CH1Config.ICPolarity=TIM_ICPOLARITY_RISING;    //上升沿捕获，捕获极性
    TIM4_CH1Config.ICSelection=TIM_ICSELECTION_DIRECTTI;
    TIM4_CH1Config.ICPrescaler=TIM_ICPSC_DIV1;          //配置输入分频，不分频，每个上升沿都会捕获
    TIM4_CH1Config.ICFilter=0;
    HAL_TIM_IC_ConfigChannel(&TIM4_Handler,&TIM4_CH1Config,TIM_CHANNEL_1);

    HAL_TIM_IC_Start_IT(&TIM4_Handler,TIM_CHANNEL_1);   // 开启TIM5的捕获通道1，并且开启捕获中断
    __HAL_TIM_ENABLE_IT(&TIM4_Handler,TIM_IT_UPDATE); // 使能更新中断,不使用更新中断，因为定时器预装值已经足够大了
    // 所谓更新中断就是普通的中断处理函数
}

int rt_hw_frequency_timer_int(void)
{	
    TIM3_CH2_Cap_Init(0xffffffff, 90 - 1);
    TIM4_CH1_Cap_Init(0xffffffff, 90 - 1);
...
void HAL_TIM_IC_MspInit(TIM_HandleTypeDef *htim)
{
    ...
    if (htim->Instance== TIM4)
    {
        ...
        HAL_NVIC_SetPriority(TIM4_IRQn, (INT_TIM4_PRIO >> 4) & 0x0F, INT_TIM4_PRIO & 0x0F);
...
void FreGatherHandleUab(void)
{
    ...
    case FRISTTIME:
        ...
        g_FreGatherUab.lastValue = HAL_TIM_ReadCapturedValue(&TIM4_Handler,TIM_CHANNEL_1);
        g_FreGatherUab.sta = PREPARE;
        break;

    case PREPARE:
        g_FreGatherUab.curValue = HAL_TIM_ReadCapturedValue(&TIM4_Handler,TIM_CHANNEL_1);
    ...
    case VALID:
		...
		g_TelemetryBaseDB[g_TelemetryBaseAddr.F_S] = (g_FreGatherUab.freValuelast - g_FreGatherUab.freValue) / 20 * 1000;
		
        g_FreGatherUab.freValuelast = g_FreGatherUab.freValue;
        break;
    }
}
...
void FreGatherHandleUCB(void)
{
    ...
    case FRISTTIME:
        ...
        g_FreGatherUCB.lastValue = HAL_TIM_ReadCapturedValue(&TIM3_Handler,TIM_CHANNEL_2);
        g_FreGatherUCB.sta = PREPARE;
        break;

    case PREPARE:
        g_FreGatherUCB.curValue = HAL_TIM_ReadCapturedValue(&TIM3_Handler,TIM_CHANNEL_2);
        ...
        {
            g_FreGatherUCB.data[g_FreGatherUCB.count] = 0xffff - g_FreGatherUCB.lastValue + g_FreGatherUCB.curValue;
        }
...
//    rt_enter_critical(); // 进入临界区
    ...
    g_TelemetryBaseDB[g_TelemetryBaseAddr.F] = g_FreGatherUab.freValue; // 写入数据缓冲区

	g_TelemetryBaseDB[g_TelemetryBaseAddr.F2] = g_FreGatherUCB.freValue;
	
    if (g_CalibrateFactor[CALIFACTOR_F] != 0)
    {
        g_TelemetryBaseDB[g_TelemetryBaseAddr.F] *= g_CalibrateFactor[CALIFACTOR_F];
    }
    else
    {
        g_TelemetryBaseDB[g_TelemetryBaseAddr.F] += 0.001f;
    }

    if (g_CalibrateFactor[CALIFACTOR_F2] != 0)
    {
        g_TelemetryBaseDB[g_TelemetryBaseAddr.F2] *= g_CalibrateFactor[CALIFACTOR_F2];
    }
    else
    {
        g_TelemetryBaseDB[g_TelemetryBaseAddr.F2] += 0.001f;
    }
    
    if (g_TelemetryBaseDB[g_TelemetryBaseAddr.F] < g_Parameter[ZERODRIFT_F])
    {
        g_TelemetryBaseDB[g_TelemetryBaseAddr.F] = 0;
    }
    if (g_TelemetryBaseDB[g_TelemetryBaseAddr.F2] < g_Parameter[ZERODRIFT_F])
    {
        g_TelemetryBaseDB[g_TelemetryBaseAddr.F2] = 0;
    }  

    if (((g_TelemetryBaseDB[g_TelemetryBaseAddr.UAB] <= 1) && (g_Parameter[CFG_PRO_VOL_N] == 0))||\
       ((g_TelemetryBaseDB[g_TelemetryBaseAddr.UCB] <= 1)&& (g_Parameter[CFG_PRO_VOL_N] == 1)))
    {
        g_TelemetryBaseDB[g_TelemetryBaseAddr.F2] = 0;
    }

    if ((((g_TelemetryBaseDB[g_TelemetryBaseAddr.UAB] >= 1) && (g_Parameter[CFG_PRO_VOL_N] == 0))||\
       ((g_TelemetryBaseDB[g_TelemetryBaseAddr.UCB] >= 1)&& (g_Parameter[CFG_PRO_VOL_N] == 1))) && g_TelemetryBaseDB[g_TelemetryBaseAddr.F2] == 0)    
    {
        g_TelemetryBaseDB[g_TelemetryBaseAddr.F2] = g_TelemetryBaseDB[g_TelemetryBaseAddr.F];
    }
	
	g_FreGatherUab.freValueProtect = g_TelemetryBaseDB[g_TelemetryBaseAddr.F];
	
    if (g_TelemetryBaseDB[g_TelemetryBaseAddr.Uab] <= 1)
    {
    ...
    {	
        if (g_TelemetryBaseDB[g_TelemetryBaseAddr.F] < 44.99f || g_TelemetryBaseDB[g_TelemetryBaseAddr.F] > 55.01f)
        {
		...
		{
			arr = (uint32_t)((float)312500.0f / (float)g_TelemetryBaseDB[g_TelemetryBaseAddr.F]) - 1; // 变频采集交流量
			ccr1 = arr / 2 ;
		}
