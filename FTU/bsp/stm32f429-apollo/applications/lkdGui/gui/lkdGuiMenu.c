...
/**
  *@brief  菜单兄弟节点插入
  *@param  pMenu 菜单
  *@param  bNode 前一个兄弟节点ID
  *@param  pNode 此插入节点信息
  *@retval 0 插入成功 1 没有找到bNode 2新节点内存分配失败
  */
void MenuTreeBNodeInsert(lkdMenuNode *bNode, lkdMenuNode *pNode)
{
	uint32_t *tNode;
	tNode = (uint32_t *)bNode->pBro;
	bNode->pBro = pNode;
	pNode->pBro = (lkdMenuNode *)tNode;
}

/**
  *@brief  菜单子节点插入
  *@param  pMenu
  *@param  fNode 前一个父节点ID
  *@param  pNode 此插入节点信息
  *@retval None
  */
...
/**
  ...
  *@param  pMenu
  *@param  node 要删除的节点ID
  *@retval 0 删除成功 1 没有找到node
  */
uint8_t MenuTreeNodeDelete(lkdMenu *pMenu, lkdMenuNode *thisNode)
{
	return 0;
}
...
/**
  ...
  */
static lkdMenuNode *GetpNodeFromIndex(lkdMenu *pM)
{
	...
	lkdCoord beginy,uint8_t index,uint8_t cNum)
{
	...
	for(uint8_t i = 0; i < cNum; i ++){
		if(indexOffset + i == index){
			...
			GuiExchangeColor();
			GuiFont12Align(x + 2,beginy + 2 + i*pM->Itemshigh,pM->ItemsWide - 3,FONT_LEFT,pthisNode->name);
			if(pthisNode->pSon != NULL){
				GuiFont12Align(x + pM->ItemsWide - 8,beginy + 2 + i*pM->Itemshigh,6,FONT_LEFT,">");
			}
			GuiExchangeColor();
		}
		else{
...
/**
  ...
  *@retval None
  */
static void RedrawMenu(lkdMenu *pM, uint8_t count)
{
