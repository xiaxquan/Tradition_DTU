...
/**
  ...
  * @updata: [YYYY-MM-DD][NAME][BRIEF]
  */
void Read_HardwareInterfaceSet(void)
{ 	
    strcpy(FileName,"/sojo");
    strcat(FileName,"/HardwareInterfaceSet.cfg");
	
    MyFile = open(FileName, O_WRONLY, 0);
    
    if (MyFile == -1)
    {
        FILE_PRINTF("NO HardwareInterfaceSet.cfg\n", MyFile);
        return;
    }
	
    read(MyFile, g_HardwareInterfaceSetDB, sizeof(struct HardwareInterfaceSetDatabase));
   
    FILE_PRINTF("f_read HardwareInterfaceSet.cfg\n", MyFile);
	...
	switch(pbuf[11])
	{
		...
		case 7:
            ...
            for(i=0;i<pbuf[12];i++)
            {
                ...
                if(!strncmp("HardwareInterfaceSet",(char *)&pbuf[i+13],sizeof("HardwareInterfaceSet") - 1))
                {
                    strcpy(Read_File[dev].filename,"/sojo");
                    strcat(Read_File[dev].filename,"/HardwareInterfaceSet.cfg");
                    break;   
                }                 
		...
		case 9:
			...
			else
			{
				...
				if(pbuf[20] ==0)
                {
                    ...
                    if(strcmp(Read_File[dev].filename,"/sojo/HardwareInterfaceSet.cfg") == 0)//读配置文件
                    {
                        Read_HardwareInterfaceSet();
                    }                      
                }
                else
                {
...
{ 
    ...
    {
        failnum++;
        FILE_PRINTF("/sojo/HardwareInterfaceSet.cfg write failed!\n");
        return 1;
    }
	
    write(MyFile,(uint8_t *)g_HardwareInterfaceSetDB, sizeof(struct HardwareInterfaceSetDatabase));
