...
{
    TYPE_WORKMODE_NONE,
    TYPE_WORKMODE_COMMON,   // 常规保护
    TYPE_WORKMODE_VOLCUR,   // 电压电流型
} TYPE_SWTICHCLASS;
...
#define ADDR_FRAM_PASSPHRASE            0x00005  // 密码存储 16个字节   
...
typedef struct TagTelesignalAddr
{
    ...
    recloseHardStrap,                               // 重合硬硬压板
    functionHardStrap,                              // 功能投退硬压板
    modBreakSoftStrap,                              // 分断软压板
    modContactSoftStrap,                            // 联络软压板
    recloseSoftStrap,                               // 重合硬软压板
    functionSoftStrap,                              // 功能投退软压板
    remote,                                         // 远方
    earth,                                          // 就地
    ...
    communication,                                  // 通讯 
    workmodeCommon,                                 // 常规保护型
    workmodeVolcur,                                 // 电压电流型
    ...
    overcurrentAccI0,                               // 零序电流后加速过流
    overcurrentAccU0,                               // 零序电压后加速过流
    ...
    overcurrentFaultIa,                           // A相过流故障
    overcurrentFaultIb,                           // B相过流故障
    overcurrentFaultIc,                           // C相过流故障
    overcurrentFaultI0,                           // 零序过流故障
    lossTrip,                                       // 失电跳闸
    getClossing,                                    // 得电合闸
    closingFaultEvent,                              // 合闸于故障
    doubleProhibitClossing,                         // 双侧有压禁止合闸
    singleLossClosingEvent,                         // 单侧失压延时合闸
    remainClockClossingUab,                       	// Uab侧残压闭锁合闸
	remainClockClossingUcb,							// Ucb侧残压闭锁合闸
...
#define TELEMETRY_BASE_TOTAL_NUM (sizeof(TelemetryAddr)/sizeof(rt_uint16_t))
#define TELEMETRY_TOTAL_NUM  (FIXED_VALUE_NUM + TELEMETRY_BASE_TOTAL_NUM)
...
enum UlogAddr
{
    ADDR_ULOG_STARTUP,          // 终端启动
	ADDR_ULOG_POWER,            // 终端供电
	ADDR_ULOG_FIXVALUE,         // 定值修改
	ADDR_ULOG_SWTICHSTA,        // 开关状态
    ADDR_ULOG_SOFTUPDATA,       // 软件更新
    ADDR_ULOG_TASKSCH,          // 任务调度
...
enum AddrRunParameter
{
    ...
    SWTICH_CLASS,                         // 开关类型
    MODBREAK_STRAP,                       // 分断软压板
    MODCONTACT_STRAP,                     // 联络软压板
    RECLOSE_STRAP,                        // 重合硬软压板
    FUNCTION_STRAP,                       // 功能投退软压板
...
//	BATTERY_LOWVOLTAGE_TIME,             // 电池低压延时	
    ...
    SLIPLOCK_SWITCH,                     // 滑差闭锁投退
	SLIPLOCK_VALUE,                      // 滑差闭锁定值
    CLOSING_LOOP_SWITCH,                 // 合环功能投退
    CLOSING_LOOP_VOLTAGE,                // 同期有压
    CLOSING_LOOP_NONVOLTAGE,             // 同期无压
    VOLTAGE_DIFFERENCE,                  // 两侧压差	
	PHASEANGLE_DIFFERENCE,				 // 相角差
    FREQUENCY_DIFFERENCE,				 // 频率差
    CLOSING_LOOP_DELAY,                  // 合环检同期时限
    CONTROL_LOOP_ABNOMAL_ENABLE,         // 控制回路异常使能	
	IACCI_SWITCH,                        // 后加速投退 软压板定值，可以选择“投入”、“退出"
	IACCI_CURRENT_VALUE,                 // 后加速电流 电流定值，整定范围：00.00s~99.99A
	IACCI_TIME,                          // 后加速时间 时间定值，整定范围：00.00s~99.99s	
    IACCI0_SWITCH,                       // 后加速投退 软压板定值，可以选择“投入”、“退出"
	IACCI0_CURRENT_VALUE,                // 后加速电流 电流定值，整定范围：00.00s~99.99A
	IACCI0_TIME,                         // 后加速时间 时间定值，整定范围：00.00s~99.99s
    IACCU0_SWITCH,                       // 后加速投退 软压板定值，可以选择“投入”、“退出"
	IACCU0_CURRENT_VALUE,                // 后加速电压 电流定值，整定范围：00.00s~99.99V
	IACCU0_TIME,                         // 后加速时间 时间定值，整定范围：00.00s~99.99s
	IACCFI_SWITCH,						 // 负荷开关合于故障后加速投退
	IACCFI_CURRENT_VALUE,				 // 负荷开关合于故障后加速定值
	...
	RECLOSE_ONE_SWITCH,					 // 重合闸一次投退
	RECLOSE_TOW_SWITCH,					 // 重合闸二次投退
    ...
    OVERCURRENTFAULT_VALUE,              // 相间过流定值
    OVERCURRENTFAULTI0_VALUE,            // 零序过流定值
	CURRENT_VALUE,                       // 有电流定值
	VOLTAGE_VALUE,                       // 电压定值
    NONVOLTAGE_VALUE,                    // 无压定值
	FAULT_CHECK_Y_TIME,                  // 故障检测Y时间
    LOCKED_CLOSING_CUR_SWITCH,           // 合闸闭锁判电流投退
    LOCKED_OPENING_SWITCH,               // 分闸闭锁投退
	SHORT_LOCKED_OPENING_TIME,           // 短时闭锁分闸时间
	LOSS_ELECTRICITY_SWITCH,             // 失电分闸投退 “1投入”、“0退出”
	LOSS_OPENING_TIME,                   // 分闸时间
	UAB_GET_VOLTAGE_CLOSSING_SWITCH,     // 电源得电合闸投退
    UCB_GET_VOLTAGE_CLOSSING_SWITCH,     // 负荷得电合闸投退
...
extern float    g_TelemetrySetValue[TELEMETRY_BASE_TOTAL_NUM];
#endif
...
extern int     g_MenuUnable;
extern int     g_MenuEnable;

extern int     g_FixedMenu0Enable;
extern int     g_FixedMenu1Enable;
extern float    *g_pFixedValue;
extern float     g_FixedValue1[FIXED_VALUE_NUM];
extern float     g_FixedValue2[FIXED_VALUE_NUM];
extern float     g_FixedValueP[FIXED_VALUE_NUM];
...
extern void rt_multi_common_data_save_value_to_fram(uint8_t sn,uint8_t check);
